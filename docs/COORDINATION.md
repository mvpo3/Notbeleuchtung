...
- 2026-09-01 **F2/Selman SYNC** — `selman/ausgaenge-zirkulation` rebased auf `b416a31` (2 Commits oben; Konflikt nur dieses Log, beide Seiten behalten). Contracts unverändert. **511 passed / 5 skipped / 1 deselected (visual)**, ruff sauber. Naht-Analyse der ~70 eingehenden Commits (72 Agenten, Befunde adversarial verifiziert):
  · **@mvpo3 (platzierung):** (a) `Ausgang.typ` wird produktiv nicht gelesen — `anker_strategy.py:79` nimmt alle Exits ohne Filter, einziger typ-Leser ist die unverdrahtete `sichtlinie`. (b) Ausgänge sind nie Graphknoten (`exit_i` ∉ `nodes`) → `distanz_zu_ausgang` leer (`graph.py:56-58`) → 14/14 Anker-Geschosse 100 % Fluchtrichtung „unten" (`anker_strategy.py:90-95`). **Vorschlag ohne Contract:** Selman hängt jeden Ausgang als Kante an den nächsten Weg-Knoten (Format wie `graph.py:26-28`/4OG-Fixture) — Abnahme F1 erbeten. (c) Regel 8 (`validierung.py:193-216`, >100 Räume & ≤2 Symbole) setzt **7/8 Mollgasse-Geschosse auf „fehler"** (0 Segmente → 0 RZ, 1–2 SL); EG „ok" (13 RZ + 21 SL). Wurzel = fehlender Fluchtweg-Layer + Wandfragmente (Selman) — aber 0 RZ bliebe auch nach Gap-Healing, solange `platzierer.py:50-55` + `anker_strategy.py:70-71` ohne Graph abbrechen.
  · **@EnisAMG (normwissen):** (a) `stair_exit` gibt es seit 504e8ff auf Fischamender/Barawitzka/Herrenholz; Mollgasse-EG bleibt 4 final_exit (Kaskade first-hit unterdrückt die 2 Stiegen-Anker) → RZ-05/RZ-07-Blocker teilweise. (b) `test_ground_truth_mollgasse.py` Schwellen ≥4 final_exit / ≥6 ist_notausgang treffen den Ist-Wert **ohne Marge** — 2 der 6 Flags sind SCHIEBETÜR-80/Fenstertür-88 (Balkon, kein Gebäudeausgang). Bereinigt Selman `_AUSSENTUER`, bricht der Test → Zählweise bitte abstimmen. (c) `mollgasse_eg.yaml:36-49` „gemessen" veraltet (HEAD: 152 Knoten / 9 Kreuzungen / 86 Segmente / 64 Richtungswechsel). (d) `platzierung_regeln.yaml:153-156` „weder stair noch stair_exit" veraltet.
  · **Selman-intern (nächste Slices):** Mollgasse 1.–4.OG liefern je 1 **final_exit** aus Stufe 2 = Balkon-Schiebetür (`tueren.py:26` SCHIEBET|FENSTERT), DG 3, 1.KG 1 stair_exit, 2.KG 0; `WDB_KLIMA-AUSSEN` wird als Tür erkannt; KG-Stempel-Bleed TECHNIK/GARAGE aus Lüftungsnotizen (`raumtyp.py:44-59`, raum_4 327 m² / raum_7 1147 m²); Herrenholz STIEGENHAUS 5→4 (raum_46 „AR" deckt Treppen-Anker, `geometrie_typ.py:133-137`); Treppen-Vokabular nur „stiegenhaus"/„trh". **Fremde Edits in `raumerkennung/`** (Leonis #47/#50/#57/#58: `raumtyp.py` `_EXTRA_LABELS`/`_EXTRA_DIRECT`, `_port/models/room.py` token-exakt) gesichtet — Bleed-Fix korrekt (1.KG GANG 2→0, Herrenholz GANG 151→123); Vokabular-Werte sind jetzt CI-Invariante (`tests/contract/test_lb_raumtyp_naht.py` ↔ `lb_extraktion.yaml:308-311`). Abnahme durch Owner offen.
