...
- 2026-09-01 **F2 → @Leonis-F1 (Platzierung): Befund aus Echt-Daten-Verifikation Mollgasse EG.** Nach #73/#74 + F1-Fix #76 den Live-Stand echt gefahren (`pipeline.run` auf `Projekte/Mollgasse/Erdgeschoß.dxf`, EG): 15 RZ + 21 SL, Prüfung **ok**, F1-Fix #76 hält (21 SL rendern als eigenes Symbol, KEINE falschen Doppelpfeile), NODEID/Belegungsliste/Koten rendern sauber. **Aber die neue Stromkreis-Belegungsliste (#74) deckt eine grobe `circuit_hint`-Vergabe auf:** alle 36 Leuchten hängen an nur **2 Kreisen** (`AGV-A-F13` 12, `AGV-B-F13` 24), beide **DL/BL gemischt** (RZ-Dauerlicht + SL-Bereitschaft auf demselben Endstromkreis). Real: DL/BL trennt man, und je Kreis gilt eine Leuchten-Obergrenze (Stromstärke 115/350 mA, §3b). → **Platzierung-Lane:** `circuit_hint`-Generierung sollte (a) nach Schaltungsart trennen und (b) je Kreis deckeln. Render-seitig ist nichts zu tun — die Belegungsliste macht's nur sichtbar (genau ihr Zweck). Kein Contract nötig für (a)/(b), reine Kreis-Zuordnungslogik.
