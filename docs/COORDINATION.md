...
- 2026-09-02 **Leonis: Symbol-Datenmodell (Digest #6/#7) — PR #96 (Contract, 3-Owner) + #98 (Konsumption, stacked).** `PlatzierungsErgebnis` **v1.2.0**: `Platzierung` trägt optional den Profi-Datensatz je Leuchte — `schaltungsart` (DL/BL, neues Literal), `luminaire_id`, `typ_letter` (TYPENUMBER), `typ_name` (TYPENAME), `montage_art`, `technologie`, `stromaufnahme_ma` (macht den strombasierten Kreis-Deckel #14 erst möglich; bis dahin bleibt der Stückzahl-Platzhalter). Alles additiv/None-Default. Konsumption (#98): `circuit_zuordnung` füllt als finaler Pass DL/BL + Leuchten-IDs (Schema der bisherigen Render-Synthese) + Legenden-Letter je `catalog_key`; der Render bevorzugt die Contract-Felder mit Fallback (Fremd-Platzierer unverändert) und rendert die **Typ-Letter-Stückliste** (Profi-Form Barawitzkagasse), sobald alle Platzierungen einen Letter tragen — Visual-Goldens unverändert (Fixture ohne Letters). 596 passed, ruff clean. **@EnisAMG/@polatselman:** #96 wartet auf eure Approvals (wie #87/#93). Offene Anschluss-Punkte: `typ_name`/`stromaufnahme_ma` brauchen Produktdaten (LB/Katalog/LDT — Enis-Lane); Prüfregel „RZ muss DL" für Fremd-Platzierer folgt separat.
- 2026-09-02 **Leonis: OIB-Stack Teil 3 — ProjektKontext über HTTP + Gate v2 raum-genau (PR #92, stacked auf #88).** `POST /plan`/`/projekt` nehmen ein optionales Form-Feld `projekt_kontext` (JSON → `ProjektKontext`, ungültig = 422); der `oib`-Block (Stufen/Gate/`raum_genau`/Hinweise) steht im `X-Notbeleuchtung`-Header und im `/projekt`-Toplevel-Summary (gleiche Begründung wie `lb_review`: fail-closed muss an der API-Grenze sichtbar sein). Contract-Basis dafür in **#87 erweitert** (Commit `72ee052`): `OibErgebnis.raum_referenzen` (additiv, v1.1.0) — der Befund echot die Raum-Zuordnung des Gebäudeteils. `oib_gate.freigegebene_raeume(oib, floor)`: tragen ALLE bestätigenden Teile Referenzen → Gate nur für die referenzierten Räume des Geschosses; fehlt einem bestätigenden Teil die Zuordnung → projekt-global (kein stilles Enger-Auslegen); fail-closed unverändert. **@EnisAMG — eine Zeile in deiner Lane:** `normwissen/oib/provider.py::_ergebnis` echot jetzt `teil.raum_referenzen` in den Befund (kein Logik-Eingriff); bitte im #92-Review mitprüfen, Veto gilt — alternativ übernimmst du die Zeile in deinen nächsten Slice und ich droppe sie. Merge-Reihenfolge #87 (3-Owner!) → #88 → #92; ohne `projekt_kontext` bleibt jeder Plan bit-identisch. 562 passed, ruff clean, Schema in sync. **PR #89 (Quellen-Korrekturen + Sonderstellen-GO) ist inzwischen gemergt.**
