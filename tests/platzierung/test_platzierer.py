...
def test_reproduziert_fuenf_rettungszeichen():
    ...
    assert len(out.platzierungen) == 5
    assert all(p.kind == "rz" for p in out.platzierungen)
...
def test_positionen_und_covers_matchen_fixture():
    ...
    got = {tuple(p.xy_mm): tuple(p.covers_segment) for p in out.platzierungen}
...
def test_naht_norm_quelle_und_catalog_key():
    ...
    for p in out.platzierungen:
        ...
        assert set(p.covers_segment)  # jede Platzierung deckt genau ihr Segment
